		...
		init: function() {
			...
			this.alloyFingerList.push(new AlloyFinger(this.croppingBox, {
				...
				pinch: function(evt) {
					...
					if((self.initScale * tempo <= 10) && (self.initcr.left - dw <= boxOffX) && (self.initcr.right + dw >= self.width + boxOffX) && (self.initcr.top + dh <= boxOffY) && (self.initcr.bottom - dh >= self.height + boxOffY)) {
